...
const paths = routes.filter(route => route.includes('/onboarding-2/step-4/'))
  ...
  render() {
    ...
    const name = (
      <Div flex={1} column padding="30px 16px">
        ...
        <Button
          onClick={() => this.props.history.push('/onboarding-2/step-4/card')}
    ...
    const card = (
      <Div flex={1} column itemsCenter padding="30px 16px">
        ...
        <Button onClick={() => this.props.history.push('/onboarding-2/step-4/contact')} style={{ marginTop: 'auto' }}>
          {texts.misc.nextStep}
        </Button>
      </Div>
    )
    ...
    const contact = (
      <Div flex={1} column padding="30px 16px">
        ...
        <Button
          onClick={() => this.props.history.push('/onboarding-2/foo')}
    ...
    return (
      <Fragment>
        <StepInfo step={4} path={this.props.location.pathname} paths={paths}>personal information</StepInfo>

        <Route path="/onboarding-2/step-4/name" render={() => name} />
        <Route path="/onboarding-2/step-4/card" render={() => card} />
        <Route path="/onboarding-2/step-4/contact" render={() => contact} />
      </Fragment>
    )
  }
}
